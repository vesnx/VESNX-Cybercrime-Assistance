# The [AbuseReport data exchange](AbuseReport.schema.json) reasonFlag enumerable value
This document provides context regarding the values used in the AbuseReport. The list is based on a 64 bit integer value on the FirewallBlockReasons enum that can be found in [Walter.BOM nuget package](https://www.nuget.org/packages?q=walter.bom)   
The Walter SDK (Website Automated Layered Threat Evaluation and Response) is a robust framework for integrating web application security in .Net applications developed by [VESNX](https://www.vesnx.com).
...
The bellow glossery is a glosary guid that can assist you in understanding the reason why a flag was assigned. 
